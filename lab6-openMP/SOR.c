/****************************************************************************
   ...
   gcc -O1 -fopenmp SOR.c -lrt -lm -o test_SOR
   OMP_NUM_THREADS=2 ./test_SOR
*/
...
void SOR_redblack_omp(arr_ptr v, int *iterations);
void SOR_ji_omp(arr_ptr v, int *iterations);
...
int main(int argc, char *argv[])
{
  ...
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
    ...
    set_arr_rowlen(v0, GHOST+n);
    clock_gettime(CLOCK_REALTIME, &time_start);
    SOR_redblack_omp(v0, iterations);
    clock_gettime(CLOCK_REALTIME, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
    convergence[OPTION][x] = *iterations;
  }
  ...
  printf("\n");
  printf("size, SOR time, SOR iters, red/black time, red/black iters, SOR_ji time, SOR_ji iters, SOR_blocked time, SOR_blocked iters, SOR_omp\n");
  {
    int i, j;
...
#pragma omp parallel for private(i, j, change) reduction(+:total_change)
    ...
    if (abs(data[(rowlen-2)*(rowlen-2)]) > 10.0*(MAXVAL - MINVAL)) {
      printf("OMP SOR: SUSPECT DIVERGENCE iter = %ld\n", iters);
      break;
    }
  }
  *iterations = iters;
  printf("    OMP SOR() done after %d iters\n", iters);
}
...
#pragma omp parallel for private(i, j, change) reduction(+:total_change)
  ...
  iters /= 2;
  *iterations = iters;
  printf("    SOR_redblack_omp() done after %d iters\n", iters);
  /* printf("ti == %ld, per iter %ld\n", ti, ti/iters); */
} /* End of SOR_redblack */
...
void SOR_ji_omp(arr_ptr v, int *iterations)
{
  ...
  while ((total_change/(double)(rowlen*rowlen)) > (double)TOL) {
    ...
    if (abs(data[(rowlen-2)*(rowlen-2)]) > 10.0*(MAXVAL - MINVAL)) {
      printf("SOR_ji_omp: SUSPECT DIVERGENCE iter = %d\n", iters);
      break;
    }
  }
  *iterations = iters;
  printf("    SOR_ji_omp() done after %d iters\n", iters);
}
...
#pragma omp parallel for private(i, j, change, ii, jj) reduction(+:total_change)
    for (ii = 1; ii < rowlen-1; ii+=BLOCK_SIZE) {
      for (jj = 1; jj < rowlen-1; jj+=BLOCK_SIZE) {
        for (i = ii; i < ii+BLOCK_SIZE; i++) {
          for (j = jj; j < jj+BLOCK_SIZE; j++) {
    ...
    if (abs(data[(rowlen-2)*(rowlen-2)]) > 10.0*(MAXVAL - MINVAL)) {
      printf("SOR_blocked_omp: SUSPECT DIVERGENCE iter = %d\n", iters);
      break;
    }
  }
  *iterations = iters;
  printf("    SOR_blocked_omp() done after %d iters\n", iters);
} /* End of SOR_blocked */
